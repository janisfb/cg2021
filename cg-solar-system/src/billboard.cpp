	...
	int outer_radius, vec3 color)
{
	...
	**/
	...
	for (int w = 0; w < width; w++)
	{
		...
		{
			img[(w * width + h) * 4 + 0] = 240.0 / 256.0; //red
			img[(w * width + h) * 4 + 1] = 89.0 / 256.0; //green
			img[(w * width + h) * 4 + 2] = 37.0 / 256.0; //blue

			/*double alpha = -(4 / (1.0 * width)) * pow(w, 2) + (4 / (1.0 * width)) * w;
			alpha += -(4 / (1.0 * height)) * pow(h, 2) + (4 / (1.0 * height)) * h;
			alpha /= 2;
			alpha *= 0.00001;*/

			double distance = sqrt(pow(w - width / 2.0, 2) + pow(h - height / 2.0, 2));

			distance = std::clamp((distance - inner_radius/1.5) / (width/3 - inner_radius/1.5), 0.0, 1.0);
			distance = 1 - (distance * distance * (3 - 2 * distance));
			//distance = -0.5 * (distance+inner_radius) + 1.0;
			//alpha = std::max(0.0, alpha);
			img[(w * width + h) * 4 + 3] = distance; //alpha
		}
	}
...
void Billboard::update_angles(const vec3& billboard_to_eye)
{
	...
	**/
	angle_x_ = (acos(billboard_to_eye.y) * 180.0) / M_PI - 90;
	angle_y_ = (atan2(billboard_to_eye.x, billboard_to_eye.z) * 180.0) / M_PI;
}
